...
//
// Mobile (<lg): the same floating white pill with the centre FAB carrying the
// Quran calligraphy; four icon+label items (two per side), active item turns
// green with a dot beneath. The notch is faked the same way — a half-disc of
// the page background over the pill.
//
  ...
  return (
    <>
      ...
      <aside className="fixed inset-y-0 start-0 z-40 hidden w-20 flex-col items-center gap-1 border-e border-black/5 bg-white py-6 dark:border-white/10 dark:bg-[#163024] lg:flex">
        <NavLink
          to="/quran"
          className="mb-4 flex h-12 w-12 items-center justify-center rounded-2xl bg-[#7A9D7A] text-white shadow"
          aria-label={t('tabs.quran')}
          title={t('tabs.quran')}
        >
          <QuranLogo size={26} />
